...
/**
 * A Lottery Algorithm.
 * Picks a random process to be performed under each run of this class.
 * @author crh13
...
public class Lottery implements Scheduler {
	 ...
	 * Picks a random number between the amount of tasks remaining and 0.
         * Chooses this task from the queue and then returns this back to the previous method.
	 ...
	 */
		...
		if (this.numberOfJobs<1) throw new SchedulerException("Empty Queue");
                ...
                int range = this.numberOfJobs;
                int randomNumber = rn.nextInt(range);
                lastJobReturned = (Job)this.queue.get(randomNumber);
		return lastJobReturned;
	}
